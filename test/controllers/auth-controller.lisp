...
(deftest-suite :controller test-auth-register-success
  (testing "Register a new user successfully"
    ...
    (let ((controller (make-instance '<auth-register-controller>)))
      (setf (gethash "username" (slot-value controller 'clails/controller/base-controller::params))
            "testuser")
      ...
      (let ((resp (response controller)))
        ...
        (ok (assoc :status resp) "Response should have status")
        (ok (string= (cdr (assoc :status resp)) "success")
            "Status should be success")
        (ok (assoc :data resp) "Response should have data")
        (let ((data (cdr (assoc :data resp))))
          (ok (assoc :username data) "Data should have username")
          (ok (string= (cdr (assoc :username data)) "testuser")
  ...
  (testing "Register fails when username is missing"
    (let ((controller (make-instance '<auth-register-controller>)))
      ...
      (let ((resp (response controller)))
        ...
        (ok (string= (cdr (assoc :status resp)) "error")
            "Status should be error")
        (ok (string= (cdr (assoc :message resp)) "Username is required")
  ...
  (testing "Register fails when email is missing"
    (let ((controller (make-instance '<auth-register-controller>)))
      ...
      (let ((resp (response controller)))
        ...
        (ok (string= (cdr (assoc :status resp)) "error")
            "Status should be error")
        (ok (string= (cdr (assoc :message resp)) "Email is required")
  ...
  (testing "Register fails when password is missing"
    (let ((controller (make-instance '<auth-register-controller>)))
      ...
      (let ((resp (response controller)))
        ...
        (ok (string= (cdr (assoc :status resp)) "error")
            "Status should be error")
        (ok (string= (cdr (assoc :message resp)) "Password is required")
  ...
  (testing "Register fails with weak password"
    (let ((controller (make-instance '<auth-register-controller>)))
      ...
      (let ((resp (response controller)))
        ...
        (ok (string= (cdr (assoc :status resp)) "error")
            "Status should be error")
        (ok (string= (cdr (assoc :message resp)) "Password validation failed")
...
(deftest-suite :controller test-auth-login-success
  (testing "Login succeeds with correct credentials"
    (let ((register-ctrl (make-instance '<auth-register-controller>)))
      (setf (gethash "username" (slot-value register-ctrl 'clails/controller/base-controller::params))
            "loginuser")
  ...
  (testing "Login fails with wrong password"
    ...
    (let ((controller (make-instance '<auth-login-controller>)))
      ...
      (let ((resp (response controller)))
        ...
        (ok (string= (cdr (assoc :status resp)) "error")
            "Status should be error")
        (ok (string= (cdr (assoc :message resp)) "Invalid email or password")
  ...
  (testing "GET /me fails when not authenticated"
    (let ((controller (make-instance '<auth-me-controller>)))
      ...
      (let ((resp (response controller)))
        ...
        (ok (string= (cdr (assoc :status resp)) "error")
            "Status should be error")
        (ok (string= (cdr (assoc :message resp)) "Not authenticated")
