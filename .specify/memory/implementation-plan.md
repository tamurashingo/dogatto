...
- **技術スタック**:
  ...
  - インフラ: Docker
- **アーキテクチャ**: 
  - clailsが各URLに対してHTMLとバンドルされたJSを配信
  - ReactがDOMをマウントして画面を生成
  - REST APIでデータ取得
  - ルーティングはサーバーサイド（clails）で実施
- **開発期間**: 約3-4ヶ月（想定）
- **計画作成日**: 2026-01-11
...
**参照**: `specs/002-authentication/`

#### Phase 2: TODO基本機能 ✅ 完了（2026-01-19）
- ✅ TODO CRUD API（Backend）
- ✅ TODO一覧表示（Frontend）
- ✅ TODO作成・編集フォーム（Frontend）
- ✅ TODO詳細表示（Frontend）
- ✅ ステータス管理（pending/completed）
- ✅ 期限設定
- ✅ 期限切れ検出

**参照**: `specs/003-todo-basic/`

#### Phase 3: タグ管理機能（Phase 4） ✅ 完了（2026-02-05）
- ✅ タグ CRUD API（Backend）
  - POST/GET/PUT/DELETE /api/v1/tags
  - タグ統計情報（total/completed/active counts）
- ✅ TODO-タグ関連付けAPI（Backend）
  - PUT /api/v1/todos/:ulid/tags
  - DELETE /api/v1/todos/:ulid/tags/:tagUlid
- ✅ タグフィルタリングAPI（Backend）
  - GET /api/v1/todos?tags=:ulids （OR条件）
...
**参照**: `specs/004-tag-management/`  
**コミット**: 16149d8

**技術詳細**:
- データベース: tags, todo_tags テーブル
- バリデーション: 名前1-50文字、色Hex形式
- 制約: ユーザーごとに名前が一意、最大10タグ/TODO
- フィルタリング: OR条件、タグなし対応
- スタイル: 6個のCSSファイル（1,402行）

#### Phase 4: ラベル管理機能 🟡 進行中（2026-02-05 - 現在）
- ✅ ラベル CRUD API（Backend）
  - POST/GET/PUT/DELETE /api/v1/labels
  - ラベル統計情報（total/used/unused counts）
  - ラベル検索（ラベル名・タグ名）
- ✅ ラベル-タグ関連付けAPI（Backend）
  - ラベル作成時のタグ関連付け
  - ラベル編集時のタグ更新
  - タグ全権をキャッシュして効率化
- ✅ TODO推定数API（Backend）
  - GET /api/v1/labels/estimate-todo-count?tag_ulids=...
  - タグのAND条件でTODO数を計算
- ✅ ラベル管理画面（Frontend）
  - ラベル一覧ページ（/labels）
  - ラベル作成・編集モーダル
  - タグ変更時の確認ダイアログ
  - 推定TODO数のリアルタイム計算
- ✅ ラベルフィルターコンポーネント（Phase 9）
  - TODOページにラベルフィルター追加
  - ラベルのタグすべてを持つTODOを表示（AND条件）
  - URLクエリパラメータ対応
- ✅ レスポンシブデザイン
- ✅ ダークモード対応
- ✅ データベーススキーマ
  - labels テーブル（label_ulid, owner_id, name, description）
  - label_tags テーブル（label_ulid, owner_id）
- 🟡 統合テスト（進行中）
- 🟡 ドキュメント整備（進行中）

**参照**: `specs/005-labels-management/`  
**コミット**: Phase 9 完了、未コミット

**技術詳細**:
- データベース: labels, label_tags テーブル
- バリデーション: 名前1-100文字、説明0-1000文字
- 制約: ユーザーごとに名前が一意、最低1タグ必須
- フィルタリング: AND条件（ラベルのすべてのタグを持つTODO）
- 検索: ラベル名・タグ名で検索、大文字小文字区別なし

### 現在の状態

**実装完了日**: Phase 9 完了（2026-02-08）  
**最新ブランチ**: 005-labels-management-phase6  
**デプロイ準備状況**: 🟡 Phase 9, 10, 11 完了後にデプロイ可能

**実装済み機能**:
1. ユーザー認証・認可
2. TODO基本機能（CRUD、ステータス、期限）
3. タグ管理機能（完全実装）
4. ラベル管理機能（Phase 1-9 完了、Phase 10-11 進行中）
5. レスポンシブデザイン
6. ダークモード
7. 統合テスト（タグ、ラベル一部）

**未実装機能**:
- ラベル機能（Phase 10-11: ルーティング、ドキュメント）
- マージ機能
- TODOコメント機能
- GitHub連携
- メール認証（仕様にあるが未実装）
...
### オプション1: TODO優先度機能
**目的**: TODOに優先度を設定し、重要度に応じた管理を可能にする
...
**推奨**: オプション2（TODO検索機能）または オプション3（TODOコメント機能）
